...
/** Browse patients in the current project (ports the V2 Patients workspace). */
          ...
          <tbody>
            {isLoading ? (
              ...
              data.items.map((patient) => (
                <tr
                  key={patient.id}
                  onClick={() => navigate(`/projects/${projectId}/patients/${patient.id}`)}
                  className="cursor-pointer border-b transition-colors hover:bg-muted/50"
                >
                  <td className="px-4 py-3 font-medium">{patient.patient_id_ext}</td>
                  <td className="px-4 py-3">
                    <Badge variant={statusBadgeVariant(patient.status)}>
                      {statusLabel(patient.status)}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums">{patient.note_count}</td>
