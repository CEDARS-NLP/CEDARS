...
def create_project(project_name, investigator_name, cedars_version):
    ...
    Returns:
        None
    """
    if db_conn.db["INFO"].find_one() is not None:
...
def get_info():
    ...
    """
    return db_conn.db.INFO.find_one_or_404()
...
def populate_annotations():
    ...
    """
    annotations = db_conn.db["ANNOTATIONS"]
...
def get_user(username):
    ...
    """
    user = db_conn.db["USERS"].find_one({"user" : username})
    return user
...
def add_user(username, password, is_admin=False):
    ...
    db_conn.db["USERS"].insert_one(info)
...
def get_search_query():
    ...
    All this data is kept in the QUERY collection.
    """
    query = db_conn.db["QUERY"].find_one({"current" : True})
    return query["query"]
...
def upload_notes(documents):
    ...
    Returns:
        None
    """
    notes_collection = db_conn.db["NOTES"]
    patient_ids = set()
    ...
    patients_collection = db_conn.db["PATIENTS"]
...
def  get_all_annotations_for_note(note_oid):
    ...
    """
    annotations = db_conn.db["ANNOTATIONS"].find({"note_id" : ObjectId(note_oid)}).sort([("note_start_index", 1)])
...
def get_annotation(annotation_id):
    ...
    """
    annotation = db_conn.db["ANNOTATIONS"].find_one({ "_id" : ObjectId(annotation_id) })
...
def get_annotation_note(annotation_id):
    ...
    annotation = db_conn.db["ANNOTATIONS"].find_one_or_404({ "_id" : ObjectId(annotation_id) })
    note = db_conn.db["NOTES"].find_one({ "_id" : annotation["note_id"] })
...
def get_patient():
    ...
    patient = db_conn.db["PATIENTS"].find_one({"reviewed" : False,
                                             "locked" : False})
...
def get_patient_annotation_ids(p_id):
    ...
    annotation_ids = db_conn.db["ANNOTATIONS"].find({"patient_id": p_id,
                                                   "reviewed" : False,
                                                   "isNegated" : False})
...
def mark_annotation_reviewed(annotation_id):
    ...
    db_conn.db["ANNOTATIONS"].update_one({"_id" : ObjectId(annotation_id)},
                                       {"$set": { "reviewed": True } })
...
def mark_patient_reviewed(patient_id, is_reviewed = True):
    ...
    db_conn.db["PATIENTS"].update_one({"patient_id" : patient_id},
...
def get_all_annotations():
    ...
    """
    annotations = db_conn.db["ANNOTATIONS"].find()
...
def get_proj_name():
    ...
    proj_info = db_conn.db["INFO"].find_one_or_404()
...
def update_project_name(new_name):
    ...
    db_conn.db["INFO"].update_one({}, { "$set": { "project": new_name } })
...
def get_curr_version():
    ...
    proj_info = db_conn.db["INFO"].find_one()
...
def add_project_user(username, password, is_admin = False):
    ...
    db_conn.db["USERS"].insert_one(data.copy())
...
def get_curr_stats():
    ...
    annotations = db_conn.db["ANNOTATIONS"].find({"isNegated" : False})
    ...
    for p_id in unique_patients:
        p_anno = db_conn.db["PATIENTS"].find_one({"patient_id" : p_id})
    ...
    lemma_dist = {}
    for anno in db_conn.db["ANNOTATIONS"].find({"isNegated" : False}):
...
def get_all_patients():
    ...
    """
    patients = db_conn.db["PATIENTS"].find()
...
def get_patient_lock_status(patient_id):
    ...
    Raises:
        None
    """
    patient = db_conn.db["PATIENTS"].find_one({"patient_id" : patient_id})
...
def get_patient_notes(patient_id):
    ...
    notes = list(db_conn.db["NOTES"].find(mongodb_search_query))
    return notes
...
def insert_one_annotation(annotation):
    ...
    Returns:
        None
    """
    annotations_collection = db_conn.db["ANNOTATIONS"]
