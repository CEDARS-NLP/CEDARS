...
@pytest.mark.parametrize("expected_result, patient_id", [
    [103, None],
    [12, 1111111111]])
def test_get_documents_to_annotate(expected_result, patient_id):
...
def test_save_query():
    ...
    assert db.save_query(query=query,
                  ...
                  exclude_negated=exclude_negated,
                  hide_duplicates=hide_duplicates,
                  skip_after_event=skip_after_event) == False
    
def test_get_info():
    assert db.get_info() is not None

@pytest.mark.parametrize("note_id, expected_result", [["UNIQUE0000000001", 1]])
def test_get_all_annotations_for_note(note_id, expected_result):
...
def test_get_annotation(note_id):
